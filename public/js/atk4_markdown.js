        ...
        markdown: function(field,image_upload_url) {
            ...
            (function ($, ShowDown, CodeMirror) {
                ...
                $(function() {
					...
					// return ig there is no such a text area
                    if (!el) return;



					// separate namespace for each editor
					that.md_editors[field] = {};

					that.md_editors[field]['current_editor_wrapper'] = $('#'+field).closest('.markdown-editor-wrapper').first();
					that.md_editors[field]['converter'] = new ShowDown.converter();
					that.md_editors[field]['editor']    = CodeMirror.fromTextArea( el[0], {
                            mode: 'markdown',
                            tabMode: 'indent',
                            lineWrapping: true
                        });

					var rendered = that.md_editors[field]['current_editor_wrapper'].find('.rendered-markdown').first();

                    // Really not the best way to do things as it includes Markdown formatting along with words
                    function updateWordCount() {
                        var wordCount = that.md_editors[field]['current_editor_wrapper'].find('.entry-word-count').first();
                        var editorValue = that.md_editors[field]['editor'].getValue();
					...
					function addButtonBar() {
						...
						that.md_editors[field]['current_editor_wrapper'].before(
							'<div class="atk-buttonset markdown-buttons">' +
						...
						var buttons = that.md_editors[field]['current_editor_wrapper'].parent().children('div.markdown-buttons').first().find('button');
						buttons.each(function(i,elem){
							$(elem).on('click',function() {
								var str = markUp($(this).attr('code'), that.md_editors[field]['editor'].getSelection());
								that.md_editors[field]['editor'].replaceSelection(str);
                    ...
                    function updateImagePlaceholders(content) {
                        ...
                        var imgPlaceholders = rendered.find('p').filter(function() {
                            return (/^(?:\{<(.*?)>\})?!(?:\[([^\n\]]*)\])(?:\(([^\n\]]*)\))?$/gim).test($(this).text());
                        });
                            ...
                            (function(){
                                ...
                                self.dropzone({
                                    ...
                                    success: function( file, response ){
                                        ...
                                        imgHolderMardown = that.md_editors[field]['current_editor_wrapper']
											.find(".CodeMirror-code").first()
											.find('pre').filter(function()
										{
                                            return (/^(?:\{<(.*?)>\})?!(?:\[([^\n\]]*)\])(?:\(([^\n\]]*)\))?$/gim).test(
												self.text()) && (self.find("span").length === 0
											);
                                        }),

                                        // Get markdown
                                        editorOrigVal = that.md_editors[field]['editor'].getValue(),
                                        nth = 0,
										...
										that.md_editors[field]['editor'].setValue( newMarkdown );
                    ...
                    function updatePreview() {
                        ...
                        preview.html(that.md_editors[field]['converter'].makeHtml(that.md_editors[field]['editor'].getValue()));

                        updateImagePlaceholders(preview.html());
                        updateWordCount();
                        updateOriginal();
                    }
                    ...
                    function updateOriginal() {
                        ...
                        var new_value = that.md_editors[field]['editor'].getValue();
                        textarea.val(new_value);
                    }

                    $(document).ready(function () {

						that.md_editors[field]['current_editor_wrapper'].find('.entry-markdown header, .entry-preview header').click(function (e) {
							that.md_editors[field]['current_editor_wrapper'].find('.entry-markdown, .entry-preview').removeClass('active');
                            $(e.target).closest('section').addClass('active');
                        });

						that.md_editors[field]['editor'].on("change", function () {
                            updatePreview();
                        });
						that.md_editors[field]['editor'].on("cursorActivity", function () {
                            dd();
                        });
                        ...
                        function syncScroll(e) {
                            ...
                            var $codeViewport = $(e.target),
                                $previewViewport = that.md_editors[field]['current_editor_wrapper'].find('.entry-preview-content'),
                                $codeContent = that.md_editors[field]['current_editor_wrapper'].find(".CodeMirror-sizer"),
                        ...
                        // TODO: Debounce
						var scroll_element = that.md_editors[field]['current_editor_wrapper'].find(".CodeMirror-scroll");

						scroll_element.on('scroll', syncScroll);

                        // Shadow on Markdown if scrolled
						scroll_element.scroll(function() {
                            if (scroll_element.scrollTop() > 10) {
								that.md_editors[field]['current_editor_wrapper'].find('.entry-markdown').addClass('scrolling');
                            } else {
								that.md_editors[field]['current_editor_wrapper'].find('.entry-markdown').removeClass('scrolling');
                            }
                        });

                        // Shadow on Preview if scrolled
						that.md_editors[field]['current_editor_wrapper'].find('.entry-preview-content').scroll(function() {
                            if (that.md_editors[field]['current_editor_wrapper'].find('.entry-preview-content').scrollTop() > 10) {
								that.md_editors[field]['current_editor_wrapper'].find('.entry-preview').addClass('scrolling');
                            } else {
								that.md_editors[field]['current_editor_wrapper'].find('.entry-preview').removeClass('scrolling');
                            }
                        });
